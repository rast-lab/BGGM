...
arma::mat stabilize_pd(const arma::mat& x, double jitter = 1e-8) {
  ...
  if (min_eig < jitter) {
    double shift = jitter - min_eig;
    sym_x += shift * arma::eye(sym_x.n_rows, sym_x.n_cols);
  }
...
arma::mat safe_inv_sympd(const arma::mat& x, double jitter = 1e-8) {
  ...
  arma::mat sym_out = arma::symmatu(out);
  arma::mat chol;
  if (!arma::chol(chol, sym_out)) {
    return stabilize_pd(sym_out, jitter);
  }
